...
def main():
    ...
    for data in config["DATASETS"]:
        ...
        for model_name in config["MODELS"]:
            ...
            # total training runtime + peak TRAINING memory
            reset_mem(device)
            sync(device)
            ...
            # Free training-only memory (gradients + Adam optimiser state) so the
            # inference-memory reading reflects inference alone, not leftovers.
            optimizer.zero_grad(set_to_none=True)
            del optimizer, trainer
            if device.type == "cuda":
                torch.cuda.empty_cache()

            #  inference latency per sample +  peak INFERENCE memory
    ...
    print("\n\n===== TASK 2: EFFICIENCY VERIFICATION MATRIX =====")
