...
def main():
    ...
    # --- frozen: load orgs weights, freeze backbone, train a fresh head only (safe transfer) ---
    set_seed()
    m = new_model(in_ch, n_cls, device)
    m.load_state_dict(torch.load(CKPT, map_location=device))    # start from orgs knowledge
    for p in m.parameters():
        p.requires_grad = False                                 # lock the WHOLE network
    m.classifier = nn.Linear(m.classifier.in_features, n_cls).to(device)  # fresh trainable head
    train(m, tr, val, LR, TRANSFER_EPOCHS, device, params=m.classifier.parameters())  # head only
    results["frozen"] = test(m, test_loader, device)
